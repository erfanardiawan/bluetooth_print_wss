...
public class BluetoothPrintPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, RequestPermissionsResultListener {
    ...
    // Add by: GOPAN
    private void connectUsb(MethodCall call, Result result) {
        Map<String, Object> args = call.arguments();
        if (args != null && args.containsKey("vendorId")) {
            ...
            try {
                vendorId = (int) args.get("vendorId");
            } catch (NullPointerException e) {
                vendorId = 0;
            }

            try {
                productId = (int) args.get("productId");
            } catch (NullPointerException e) {
                productId = 0;
            }

            final UsbDevice usbDevice = adapter.selectDevice(vendorId, productId);

            if (usbDevice != null) {
//                final boolean resultCon = connectUsbPort(usbDevice);
                result.success(true);
//                return;
            } else {
...
//            this.curMacAddress = String.valueOf(vendorId) + productId;
